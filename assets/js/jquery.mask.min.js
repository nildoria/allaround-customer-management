window.onload = function () {
  ...
  const securitycode = document.getElementById("cvvCode");
  ...
  function initExpirationDateMask(inputElement) {
    const currentYY = new Date().getFullYear() % 100;
    const expirationMask = new IMask(inputElement, {
      mask: "MM{/}YY",
      groups: {
        YY: new IMask.MaskedPattern.Group.Range([currentYY, 99]),
        MM: new IMask.MaskedPattern.Group.Range([1, 12]),
      },
    });

    expirationMask.on("accept", function () {
      if (expirationMask.value.length == 0) {
        document.getElementById("svgexpire").innerHTML = "MM/YY";
      } else {
        document.getElementById("svgexpire").innerHTML = expirationMask.value;
      }
    });
  }

  function initSecurityCode(elm) {
    const securitycodeMask = new IMask(elm, {
      mask: "000",
    });
  ...
  function initCardNumber(elm) {
    // console.log("Mask the Credit Card Number Input");
    const cardnumber_mask = new IMask(elm, {
      mask: [
        {
    ...
    cardnumber_mask.on("accept", function () {
      var ccicon = document.getElementById("ccicon");
      var ccsingle = document.getElementById("ccsingle");
      // console.log("cardtype", cardnumber_mask.masked.currentMask.cardtype);
  ...
  // Delegated event handling for the credit card form click event
  const flipcardElement = document.querySelector(".creditcard");

  if (flipcardElement) {
    flipcardElement.addEventListener("click", function () {
  ...
  document.addEventListener("input", function (event) {
    ...
    //     document.getElementById("svgname").innerHTML = "Your Name";
    //     document.getElementById("svgnameback").innerHTML = "Your Name";
    //   } else {
    //     document.getElementById("svgname").innerHTML = target.value;
    //     document.getElementById("svgnameback").innerHTML = target.value;
    //   }
    // }

    if (target.id === "expirationDate") {
      initExpirationDateMask(target);
    }

    if (target.id === "cvvCode") {
      initSecurityCode(target);
    }

    if (target.id === "cardNumber") {
      // setTimeout(() => {
        initCardNumber(target);
      // }, 0);

      if (target.value.length === 0) {
        document.getElementById("svgnumber").innerHTML = "1111 1111 1111 1111";
      } else {
        document.getElementById("svgnumber").innerHTML = target.value;
      }
    }
  });
  ...
  //   if (name.value.trim() === "") {
  //     // If the input is empty or contains only whitespace, set the names to a default value
  //     document.getElementById("svgname").innerHTML = "Your Name";
  //     document.getElementById("svgnameback").innerHTML = "Your Name";
  //   } else {
  //     // If the input has content, set the names to the input value
  //     document.getElementById("svgname").innerHTML = this.value;
  //     document.getElementById("svgnameback").innerHTML = this.value;
  //   }
  // });

  jQuery(document).on("click", ".alarnd--single-payout", function () {
    // console.log("dispatchEvent");
    name.dispatchEvent(new Event("input"));
  });

  // Delegated event handling for credit card number changes
  mainWrap.addEventListener("accept", function (event) {
    const target = event.target;
    ...
    if (target.id === "cardNumber") {
      if (target.value.length === 0) {
        ...
        document.getElementById("svgnumber").innerHTML = cardNumberInput.value;
      }
    }
  });

  // Function to update the expiration date display
  const updateExpirationDateDisplay = () => {
  ...
  // Delegated event handling for 'accept' event on the form
  mainWrap.addEventListener("accept", function (event) {
  ...
  jQuery(document).on(
    ...
    function () {
      jQuery(".creditcard").removeClass("flipped");
    }
  );

  jQuery(document).on("focus", "#cvvCode", function () {
    jQuery(".creditcard").addClass("flipped");
  });
};

var cardNumberInput = document.getElementById("cardNumber");
var expirationDateInput = document.getElementById("expirationDate");
var cvvCodeInput = document.getElementById("cvvCode");

if (cardNumberInput) {
  cardNumberInput.addEventListener("keyup", function (e) {
    var maxLength = parseInt(cardNumberInput.getAttribute("maxlength"), 10);
    var myLength = cardNumberInput.value.length;

    if (myLength >= maxLength && expirationDateInput) {
...
if (expirationDateInput) {
  expirationDateInput.addEventListener("keyup", function (e) {
    var maxLength = parseInt(expirationDateInput.getAttribute("maxlength"), 10);
    var myLength = expirationDateInput.value.length;

    if (myLength >= maxLength && cvvCodeInput) {
      cvvCodeInput.focus();
    }
  });
}
