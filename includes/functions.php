...
// Display ACF user field under billing details on WooCommerce order details page
add_action('woocommerce_admin_order_data_after_billing_address', 'display_custom_data_on_order_page', 10, 1);

function display_custom_data_on_order_page($order)
{
    // Get user ID from the order
    $user_id = $order->get_user_id();

    $invoice_name = get_field('invoice', 'user_' . $user_id);

    if ($invoice_name) {
        echo '<p><strong>' . __('Invoice Name') . ':</strong> ' . esc_html($invoice_name) . '</p>';
    }

    // Get user data for the user ID
    $user_data = get_userdata($user_id);

    if ($user_data) {
        $username = $user_data->user_login;

        // Generate the URL based on home_url
        $user_url = home_url('/' . $username);

        // Display the username as a clickable link
        echo '<p><strong>MiniSite Username:</strong> <a href="' . esc_url($user_url) . '">' . esc_html($username) . '</a></p>';
    } else {
        echo '<p><strong>Profile Username:</strong> User not found</p>';
    }
}
            ...
            <div class="alarnd--payout-col alrnd_tokenized_col">
                <div class="alarnd--payout-col alrnd_wooz_col">
                    <div class="alrnd--pay_details_tokenized">
                        <h2>
                            <?php esc_html_e('פרטי תשלום', 'hello-elementor'); ?>
                        </h2>
                            ...
                            <div class="alarnd-payout-choose">
                                <div class="alarnd--single-payout">
                                    <label>
                                        ...
                                        <div class="alarnd--four-digit">
                                            <?php echo esc_html($four_digit); ?>&nbsp;<span>****&nbsp;****&nbsp;****</span>
                                        </div>
                                    </label>
                                </div>
                                <div class="alarnd--single-payout">
                                    ...
                                    <svg version="1.1" id="cardfront" xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <g id="Front">
                                            ...
                                            <text transform="matrix(1 0 0 1 60.106 295.0121)" id="svgnumber"
                                                class="st2 st3 st4">1111 1111 1111 1111</text>
                                            <text transform="matrix(1 0 0 1 54.1064 428.1723)" id="svgname"
                                                class="st2 st5 st6">JOHN DOE</text>
                                            <text transform="matrix(1 0 0 1 54.1074 389.8793)" id="svgnameTitle"
                ...
                <div
                    class="alarnd--card-details-wrap <?php echo $is_disabled === false ? 'pix_all_user_data_filled' : ''; ?>">
...
function allaround_card_form($user_id = '')
{
            ...
            <?php if ($is_disabled === false): ?>
                <div id="alarnd__details_preview">
                    <div class="alarnd--payout-col alarnd--details-previewer">
                        ...
                        <div class="tokenized_inv_name_cont">
                            <?php esc_html_e('חשבונית על שם', 'hello-elementor'); ?>:
                            <?php echo !empty($invoice) ? '<p class="tokenized_user_name">' . esc_html($invoice) . '</p>' : '<p class="tokenized_user_name">' . esc_html($name) . '</p>'; ?>
                        </div>
    ...
    <form action="" id="cardDetailsForm"
        class="allaround--card-form cardForm-wCard <?php echo $is_disabled === false ? 'pix_all_user_data_filled' : ''; ?>"
        ...
        <div class="allaround_carf_form-fields">
            <div class="allaround_carf_form-cardDetail">
                ...
                <?php if (!is_user_logged_in() || empty($token)): ?>
                    <h2>
                        <?php esc_html_e('מעובד לקופה', 'hello-elementor'); ?>
                    </h2>
                <?php endif; ?>

                <div class="ministore_available_pay_options">
                    <span>
                        <?php esc_html_e('יארשא סיטרכ', 'hello-elementor'); ?>
                    </span> <img src="<?php echo esc_url($available_card_path); ?>" alt="Available Payment Options"
                        loading="lazy">
                </div>
                <div class="form-cardDetail-fields">
                    <div class="form-row">
                        <div class="form-label">
                            <?php esc_html_e("Card Number", "hello-elementor"); ?>
                        </div>
                        <div class="form-input">
                            <input type="text" id="cardNumber" name="cardNumber" maxlength="19"
                                placeholder="<?php esc_attr_e("1111 1111 1111 1111", "hello-elementor"); ?>" dir="ltr"
                    ...
                    <div class="form-row flex-row exp-cvc-con">
                        <div class="form-row">
                            <div class="form-label">
                                <?php esc_html_e("Expiration Date", "hello-elementor"); ?>
                            </div>
                            <div class="form-input">
                                <input type="text" inputmode="numeric" id="expirationDate" maxlength="5"
                                    name="expirationDate" placeholder="<?php esc_attr_e("MM/YY", "hello-elementor"); ?>"
                                    required>
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-label">
                                <?php esc_html_e("CVC", "hello-elementor"); ?>
                            </div>
                            <div class="form-input">
                                <input type="number" id="cvvCode" maxlength="3" name="cvvCode"
                                    placeholder="<?php esc_attr_e("CVC", "hello-elementor"); ?>" inputmode="numeric"
                                    required>
                                <div class="cvc-info tooltip-left"
                                    data-tooltip="<?php esc_html_e('3 סיטרכה בגב תורפס', 'hello-elementor'); ?>">
                                    ...
                                    <svg version="1.1" id="cardfront" xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <g id="Front">
                                            ...
                                            <text transform="matrix(1 0 0 1 54.1074 389.8793)" class="st7 st5 st8"
                                                id="svgnameTitle">cardholder name</text>
                                            <text transform="matrix(1 0 0 1 479.7754 388.8793)"
                                                class="st7 st5 st8">expiration</text>
                                            <text transform="matrix(1 0 0 1 65.1054 241.5)" class="st7 st5 st8">card
                                                number</text>
...
function allaround_customer_form($is_disabled = false)
{
    ...
    <form action="" id="customerDetails"
        ...
        <div class="form-row form-submit-row">
            <button type="submit" class="button alarnd--regular-button alt ml_add_loading ml_save_customer_info" disabled>
                <?php esc_html_e("Update", "hello-elementor"); ?>
            </button>
            <a href="#" class="ml_customer_info_edit_cancel">
                <?php esc_html_e("Return", "hello-elementor"); ?>
            </a>
        </div>
...
function add_address_phone_fields_to_user_form()
{
    ...
    if ($pagenow === 'user-new.php') {
        ...
        <tr>
            ...
            <td>
                <input type="hidden" name="xoo-ml-user-reg-phone-cc" id="xoo-ml-user-reg-phone-cc"
...
function ml_create_order($data)
{
    ...
    foreach ($products as $product) {
        ...
        if (!empty($wc_thumb)) {
            ...
            $ext = $filetype['ext'];
            $filename_only = 'wc_thumb_' . $product_id . '.' . $ext;

            $colors = get_field('color', $product_id);
            if ($alarnd_color_key !== '' && isset($colors[$alarnd_color_key]) && !empty($colors[$alarnd_color_key])) {
...
function ml_get_filter_content($current_user_id, $filter = '', $pagination = true)
{
                            ...
                            <div class="pricingDescSteps">
                                                ...
                                                foreach ($discount_steps as $step):
                                                    ...
                                                    <div class="alarnd--price-chart-item">
                                                        <span class="price_step_price">
                                                            <?php echo $step['amount'] == 0 ? wc_price($product->get_regular_price(), array('decimals' => 0)) : wc_price($step['amount'], array('decimals' => 0)); ?>
                                                        </span>
                                                        <span class="price_step_qty">כמות: <span>
                                ...
                                <div class="modal-bottom-btn">
                                    <button type="button" class="alarnd_trigger_details_modal ml_add_loading"
                                        ...
                                        <?php esc_html_e('הוסיפו לעגלה', 'hello-elementor'); ?>
                                    </button>
                                </div>
                            </div>
                        </div>
    ...
    if ($totalItems >= $itemsPerPage):
        ...
        <div class="allaround--loadmore-wrap">
            <button type="button" class="alarnd--regular-button alarnd--loadmore-trigger ml_loading ml_add_loading button"
                data-page_num="1">
                <?php esc_html_e("Load More", "hello-elementor"); ?>
            </button>
            <button type="button" class="alarnd--regular-button alarnd--reveal-more ml_add_loading button">
                <?php esc_html_e("מוצרים נוספים...", "hello-elementor"); ?>
            </button>
        </div>
        <?php
    endif;
...
/**
 * Check if the value is present in the given array of arrays.
 *
 * @param mixed $valueToCheck The value to search for in the arrays.
 * @param array $yourArray The array of arrays to search in.
 * @return bool
 */
function MlIsValueInArray($valueToCheck, $yourArray)
{
    foreach ($yourArray as $subArray) {
        if (isset($subArray['value']) && $subArray['value'] == $valueToCheck) {
            return true;
        }
    }
    return false;
...
/**
 * Get users who have purchased a specific product.
 *
 * @param int $product_id The ID of the product.
 * @return array The list of customers who have purchased the specified product.
 */
function ml_get_users_by_product($product_id)
{
    $all_customers = ml_customer_list();

    // filter user that has selected this product
    $filter_user = array();
    foreach ($all_customers as $customer_id) {
        ...
        $selected_ids = ml_get_user_products($customer_id);
        if (empty($selected_ids) || !MlIsValueInArray($product_id, $selected_ids))
            continue;

        $user_data = ALRN_Genrator::instance()->get_user_data($customer_id, false, $product_id);

        if (empty($user_data))
            continue;

        $filter_user[$customer_id] = $user_data;
    }
